...
slideshowServer = http.createServer(function(req, res){
  ...
  fs.readFile(fileName, function(err, data) {
    ...
    if (path.match(/\.js/)) {
        ...
        res.writeHead(200, {'Content-Type': 'application/x-woff'});
...
io.on('connection', function(client) {
  sys.puts('Connection created [id:' + client.sessionId + '] [time:' + Date.now() + ']');
  ...
  client.on('disconnect', function() {
    sys.puts('Connection closed [id:' + client.sessionId + '] [time:' + Date.now() + ']');
  });
});
